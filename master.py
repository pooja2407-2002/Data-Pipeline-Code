...
s3jobdata = spark.read.load("s3://z43n/dest/ipCount")

snowjobdata = spark.read.load("s3://z43n/dest/custScores")

webapijobdata = spark.read.load("s3://z43n/dest/customerApi")


joindf = webapijobdata.join(s3jobdata , ["username"] , "inner").join(snowjobdata , ["username"] ,"inner")


joindf.show()

joindf.write.format("parquet").mode("overwrite").save("s3://z43n/dest/master")
